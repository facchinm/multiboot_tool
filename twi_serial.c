...
#define TWI_DEFAULT_DEVICE  "/dev/ttyACM0"
...
static void twi_serial_close_device(struct twi_privdata *twi)
{
    if (twi->connected)
        sp_close(twi->port);

    twi->connected = 0;
    sp_free_port(twi->port);
}

static int twi_serial_open_device(struct twi_privdata *twi)
{
    enum sp_return ret;
    sp_get_port_by_name(twi->device, &(twi->port));
    ret = sp_open(twi->port, SP_MODE_READ | SP_MODE_WRITE);
    if (ret != SP_OK) {
...
static int twi_serial_close(struct multiboot *mboot)
{
    struct twi_privdata *twi = (struct twi_privdata *)mboot->privdata;

    if (twi->connected)
        twi_serial_switch_application(twi, BOOTTYPE_APPLICATION);

    twi_serial_close_device(twi);
    return 0;
}

static int twi_serial_open(struct multiboot *mboot)
{
    struct twi_privdata *twi = (struct twi_privdata *)mboot->privdata;
    ...
    if (twi_serial_open_device(twi) != 0)
        return -1;

    if (twi_serial_switch_application(twi, BOOTTYPE_BOOTLOADER)) {
        fprintf(stderr, "failed to switch to bootloader (invalid address?): %s\n", strerror(errno));
        twi_serial_close(mboot);
        return -1;
    }
    ...
    char version[16];
    if (twi_serial_read_version(twi, version, sizeof(version))) {
    ...
    uint8_t chipinfo[8];
    if (twi_serial_read_memory(twi, chipinfo, sizeof(chipinfo), MEMTYPE_CHIPINFO, 0x0000)) {
...
static int twi_serial_read(struct multiboot *mboot, struct databuf *dbuf, int memtype)
{
    struct twi_privdata *twi = (struct twi_privdata *)mboot->privdata;
    ...
    while (pos < size) {
        ...
        if (twi_serial_read_memory(twi, dbuf->data + pos, len, memtype, pos)) {
...
static int twi_serial_write(struct multiboot *mboot, struct databuf *dbuf, int memtype)
{
    struct twi_privdata *twi = (struct twi_privdata *)mboot->privdata;
    ...
    while (pos < dbuf->length) {
        ...
        if (twi_serial_write_memory(twi, dbuf->data + pos, len, memtype, pos)) {
            mboot->progress_cb(progress_msg, -1, -1);
            return -1;
        }
...
static int twi_serial_verify(struct multiboot *mboot, struct databuf *dbuf, int memtype)
{
    struct twi_privdata *twi = (struct twi_privdata *)mboot->privdata;
    ...
    while (pos < dbuf->length) {
        ...
        if (twi_serial_read_memory(twi, comp, len, memtype, pos)) {
            mboot->progress_cb(progress_msg, -1, -1);
            return -1;
        }
...
static int twi_serial_optarg_cb(int val, const char *arg, void *privdata)
{
    ...
    case '?': /* error */
            fprintf(stderr, "Usage: twiboot [options]\n"
                ...
                "  -d <device>                  - selects i2c device  (default: /dev/ttyACM0)\n"
...
static struct multiboot * twi_serial_alloc(void)
{
    ...
    optarg_register(twi_optargs, ARRAY_SIZE(twi_optargs), twi_serial_optarg_cb, (void *)twi);

    mboot->privdata = twi;
    return mboot;
}

static void twi_serial_free(struct multiboot *mboot)
{
    struct twi_privdata *twi = (struct twi_privdata *)mboot->privdata;
...
static int twi_serial_get_memtype(struct multiboot *mboot, const char *memname)
{
...
static int twi_serial_get_memsize(struct multiboot *mboot, int memtype)
{
    struct twi_privdata *twi = (struct twi_privdata *)mboot->privdata;
...
struct multiboot_ops twi_serial_ops = {
    .alloc          = twi_serial_alloc,
    .free           = twi_serial_free,
    .get_memtype    = twi_serial_get_memtype,
    .get_memsize    = twi_serial_get_memsize,

    .open           = twi_serial_open,
    .close          = twi_serial_close,
    .read           = twi_serial_read,
    .write          = twi_serial_write,
    .verify         = twi_serial_verify,
};
